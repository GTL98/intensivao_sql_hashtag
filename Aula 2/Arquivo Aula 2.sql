...
SELECT * FROM alugueis;  -- 578 aluguéis
...
SELECT
	DISTINCT genero
FROM
	filmes;  -- há 7 gêneros de filmes no banco de dados
...
SELECT
	DISTINCT estado
FROM
	clientes;  -- somente 11 estados possem esse serviço, a empresa de streaming pode expandir seus serviços para mais 16 estados
...
LIMIT 1;  -- Natalia Guedes (2017/01/13)
...
SELECT 
    nome_ator AS 'Nome Ator', nacionalidade AS 'Nacionalidade'
FROM
    atores
WHERE
    nacionalidade IN ('Austrália' , 'Canadá', 'Irlanda do Norte')
...
SELECT 
    titulo AS 'Título do Filme', duracao AS 'Duração'
FROM
    filmes
WHERE
    duracao BETWEEN 90 AND 100
ORDER BY duracao ASC;
...
SELECT
	COUNT(id_aluguel) AS 'Qtd. Alugueis'
FROM
	alugueis;
...
SELECT
	COUNT(nota) AS 'Qtd. Notas'
FROM
	alugueis;


-- c) Você viu alguma diferença no resultado? O que aconteceu?
	-- Como a função COUNT() não conta valores nulos, esses valores na coluna 'nota' não são considerados.
...
SELECT 
    COUNT(DISTINCT genero) AS 'Qtd. Gêneros'
FROM
    filmes;
...
SELECT
	MAX(duracao) AS 'Max Duração',
    MIN(duracao) AS 'Min Duração'
FROM
	filmes;
...
SELECT 
    AVG(nota) AS 'Média de Satisfação'
FROM
    alugueis;
